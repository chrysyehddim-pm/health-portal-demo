...
PR 是每個互動項目上方唯一的近期表現摘要，顯示對應 LV、近 90 天表現範圍與前一日資料更新時間；資料每日更新一次。正式實作可由既有的完成狀況與完成時間資料換算，並以相同遊戲與挑戰強度的紀錄作為參考。會員畫面不把 PR 稱為分數，也不顯示額外的綜合分數。完成秒數只在「近期完成時間」圖表與「最近互動紀錄」中呈現；不同挑戰強度不作進步、退步或好壞解讀。
