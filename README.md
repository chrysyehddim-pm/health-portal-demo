...
- `terms.html`：首次授權／同意頁
- `group.html`：群組頁
- `records.html`：P1.5 我的動腦紀錄，可切換四款遊戲查看近期完成時間、Level 與同齡 PR
...
動腦紀錄 Prototype 的本月總覽、遊戲清單、近期完成紀錄、同齡 PR 與徽章集中設定於 `assets/data.js` 的 `brainRecords` 區塊；目前皆為展示用假資料，不串接 API。

同齡 PR 以相同遊戲、相同 Level、相同年齡區間為比較條件，近 90 天每位會員只取一筆個人最佳完成時間；同秒數採中間排名，公式為 `(較慢人數 + 0.5 × 同秒數人數) ÷ 比較人數 × 100`。Prototype 設定至少 30 位有效參考會員才顯示 PR，個人紀錄可即時更新，同齡母體預計每日批次更新一次。會員畫面不顯示參考人數。
...
徽章情境沿用同一份 HTML 與 JavaScript，透過 `stage=p16` 網址參數切換，避免維護兩份重複頁面。群組徽章名稱為「健康同行」，條件是成功建立群組且至少有一位其他會員完成加入；取得後永久保留。
